...
import com.hyh.mallchat.common.common.utils.FutureUtils;
import com.hyh.mallchat.common.common.utils.discover.UrlDiscover;
...
public abstract class AbstractUrlDiscover implements UrlDiscover {
    ...
    public Map<String, UrlInfo> getUrlContentMap(String content) {
        ...
        CompletableFuture<List<Pair<String, UrlInfo>>> listCompletableFuture = FutureUtils.sequenceNonNull(futures);

        return listCompletableFuture.join().stream().collect(Collectors.toMap(Pair::getFirst, Pair::getSecond, (a, b) -> a));
    }
    /**
     * 判断链接是否有效
     *
    ...
    private Document getUrlDocument(String match) {
        try {
            Connection connect = Jsoup.connect(match);
            connect.timeout(2000);
            return connect.get();
        } catch (Exception e) {
            log.error("获取url内容失败", e);
        }
        return null;
    }
